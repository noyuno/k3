...
if [[ -z "${RCLONE_CRONFILE:-}" ]]; then
  ...
  if [[ -n "${RCLONE_CROND_SOURCE_PATH:-}" ]] || [[ -n "${RCLONE_CROND_DESTINATION_PATH:-}" ]]; then
    ...
    if [[ ! -z ${RCLONE_CROND_SOURCE_SIZE:-} ]]; then
      {
        echo 'check program foldersize with path "/bin/bash -c '${BASEDIR}/rclone.sh foldersize'"'
        echo '    if status != 0 for 2 cycles then exec "/usr/bin/env bash -c '${BASEDIR}/rclone.sh run'"'
    ...
    if [[ -z ${RCLONE_CROND_HEALTHCHECK_URL:-} ]]; then
      {
        ...
        echo '{{RCLONE_CROND_SCHEDULE}} /usr/bin/env bash -c "'${BASEDIR}'/rclone.sh run sync" >'$log' 2>&1'
      } | tee ${RCLONE_CRONFILE}
    ...
    else
      {
        echo 'SHELL=/bin/bash'
        echo 'PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin'
        echo '{{RCLONE_CROND_SCHEDULE}} /usr/bin/env bash -c "'${BASEDIR}'/rclone.sh run sync" >'$log' 2>&1 && curl -fsS --retry 3'
